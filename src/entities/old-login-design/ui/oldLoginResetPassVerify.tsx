...
const OldLoginResetPassVerify: React.FC<Props> = ({
  onSubmit,
  isLoading,
  error
}) => {
  ...
  return (
    <div className={styles.OldLoginFormContainer}>
      <OldLoginWrapper text={t("terminalLogin.resetFormHeading", { step: "otp" }) as string}>
        {error && (
          <span className={styles.error}>{error}</span>
        )}
        <OldLoginInput
          name="email"
          placeholder={t('placeholders.resetOtp') as string}
          value={otp}
...
export default OldLoginResetPassVerify;
